...
class _CurvePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    size = Size(size.width, size.height * 1.8);
    ...
    canvas.drawPath(
        Path()
          ..addOval(Rect.fromCircle(
              center: handler, radius: appearance.handlerSize + 5))
          ..fillType = PathFillType.evenOdd,
        Paint()
          ..color = Colors.black.withAlpha(30)
          ..maskFilter =
              MaskFilter.blur(BlurStyle.normal, convertRadiusToSigma(25)));

    dotPaint.color = appearance.dotBorderColor;
    canvas.drawCircle(handler, appearance.handlerSize + 5, dotPaint);

    dotPaint.color = appearance.dotColor;
