...
Future facebookLogin() async {
    ...
    if (result.status == LoginStatus.success) {
     ...
     // _userData = userData;
      ...
      // firebaseAuth.signInWithProvider(facebookProvider);

      final CollectionReference userCollection  = FirebaseFirestore.instance.collection("users");
      QuerySnapshot checkUserSnapshot = await userCollection.where("uid", isEqualTo: FirebaseAuth.instance.currentUser!.uid).get();
      if(checkUserSnapshot.docs.isEmpty){
        await DatabaseService(uid: FirebaseAuth.instance.currentUser!.uid).savingUserData(userData['name'], userData['email']);
      }
      QuerySnapshot snapshot = await DatabaseService(uid: FirebaseAuth.instance.currentUser!.uid).gettingUserData(userData['email']);
      await HelperFunctions.saveUserLoggedInStatus(true);
      await HelperFunctions.saveUserEmailSF(userData['email']);
      await HelperFunctions.saveUserNameSF(snapshot.docs[0]['userName']);
      return true;
      }
      catch(e){
